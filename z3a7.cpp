...
public:
   ...
   */
     ...
     */
    LZWBinFa& operator<< (char b)		//eltolási operátor
    {
	...
	// ha a void helyett 'LZWBinFa' van, akkor ide a végére: return *this;
		return *this;
    }
...
main (int argc, char *argv[])
{
	...
	//vagy:  std::cout<<*(argv[1]+1)<<std::endl;
	//vagy:  std::cout<<argv[1][1]<<std::endl;
    ...
    char *inFile = *++argv;	//EDDIG: infile-ra mutat

    // a -o kapcsoló jön?
    if (*((*++argv) + 1) != 'o')	//+1 MIATT: az o-ra fog mutatni
    {
        usage ();
        return -2;
    }
    ...
    while (beFile.read ((char *) &b, sizeof (unsigned char)))		//utolsó betűjel első karakterére mutat
    {
    ...
    operator<< (kiFile,binFa); //2 eltolás
